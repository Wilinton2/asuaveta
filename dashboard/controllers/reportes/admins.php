...
$sql = "SELECT id_usu, nombre_usu, apellido_usu, correo_usu, telefono_usu, fecha_registro_usu, fk_rol_usu FROM tbl_usuario WHERE fk_rol_usu=2 ORDER BY fk_rol_usu DESC";
...
$gerente = $cons_gerente->fetch(PDO::FETCH_OBJ);
    ...
    $pdf->setY(35);
    $pdf->setX(90);
    $pdf->Image('../../../assets/img/user_admin.jpg', 98, 40, 14);
    ...
    $pdf->Ln();
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(190, 5, 'REPORTE DE ADMINISTRADORES', 0, 0, 'C');
    $pdf->Ln(6);
    $pdf->SetFont('Arial', '', 9);
    $pdf->Cell(190, 5, 'Los administradores registrados en nuestro sistema ASUAVETA hasta la fecha '.strftime("%d de %B del %Y").' son:', 0, 0, 'C');
